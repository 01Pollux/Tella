...
static inline PVOID GetAddress__(VoidFunc fn)
{
...
#define DETOUR_LINK_TO_MEMBER(NAME, ADDRESS) \
		...
		DetourAttach(&reinterpret_cast<PVOID&>(NAME##_CLASS::NAME##_ACTUAL), GetAddress__(reinterpret_cast<VoidFunc>(&(NAME##_CLASS::NAME)))); \
		LONG error = DetourTransactionCommit()

#define DETOUR_UNLINK_FROM_MEMBER(NAME) \
		DetourTransactionBegin(); \
		DetourUpdateThread(GetCurrentThread()); \
		DetourDetach(&reinterpret_cast<PVOID&>(NAME##_CLASS::NAME##_ACTUAL), GetAddress__(reinterpret_cast<VoidFunc>(&(NAME##_CLASS::NAME)))); \
		LONG error = DetourTransactionCommit()
...
#define DETOUR_LINK_TO_STATIC(NAME, ADDRESS) \
		...
		DetourTransactionBegin(); \
		DetourUpdateThread(GetCurrentThread()); \
		DetourAttach(&reinterpret_cast<PVOID&>(NAME##_ACTUAL), NAME); \
		LONG error = DetourTransactionCommit()

#define DETOUR_UNLINK_FROM_STATIC(NAME, ADDRESS) \
		DetourTransactionBegin(); \
		DetourUpdateThread(GetCurrentThread()); \
		DetourDetach(&reinterpret_cast<PVOID&>(NAME##_ACTUAL), NAME); \
		LONG error = DetourTransactionCommit()
...
public:
    explicit IDetour(void* addr, void* callback): callback(callback), actual(addr)
    {
        DetourTransactionBegin();
        DetourUpdateThread(GetCurrentThread());
        DetourAttach(&actual, callback);
