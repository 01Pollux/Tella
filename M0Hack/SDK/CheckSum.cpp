...
static_assert(std::extent_v<decltype(pulCRCTable)> == 256);
...
void CRC32_ProcessBuffer(CRC32_t* pulCRC, const void* pBuffer, int nBuffer)
{
    ...
    const uint8_t* pb = static_cast<const uint8_t*>(pBuffer);
...
JustAfew:
    ...
    uint32_t nFront = (reinterpret_cast<uint32_t>(pb)) & 3;
    nBuffer -= nFront;
    switch (nFront)
    {
    case 3:
...
// Output : static void
//-----------------------------------------------------------------------------
static void MD5Transform(unsigned int buf[4], const uint32_t in[16])
{
    uint32_t a, b, c, d;
...
//			*ctx - 
//-----------------------------------------------------------------------------
void MD5Final(unsigned char digest[MD5_DIGEST_LENGTH], MD5Context* ctx)
{
...
unsigned int MD5_PseudoRandom(unsigned int nSeed)
{
    ...
    return *reinterpret_cast<uint32_t*>(digset.bits + 6);	// use 4 middle bytes for random value
}