...
class M0Color_Wrapper : public CustomArray<char8_t, 4, HasNotifier>
{
	using color_type = color::u8rgba;
	using base_type = CustomArray<color_type::type, 4, HasNotifier>;
	M0CONFIG_INHERIT_FROM(M0Color_Wrapper, color_type, base_type);
...
using U8Color = M0Color_Wrapper<false>;
using U8ColorN = M0Color_Wrapper<true>;
