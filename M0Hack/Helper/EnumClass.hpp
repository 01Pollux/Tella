...
namespace tella
{
    ...
    public:
        ...
        _NODISCARD bitmask operator~()              const noexcept { return ebitset(bitset.operator~()); }
        _NODISCARD bitmask operator<<(type pos)     const noexcept { return ebitset(bitset.operator<<(static_cast<size_t>(pos))); }
        _NODISCARD bitmask operator>>(type pos)     const noexcept { return ebitset(bitset.operator>>(static_cast<size_t>(pos))); }
        _NODISCARD bool operator==(type pos)     const noexcept { return bitset.operator==(static_cast<size_t>(pos)); }

        bitmask& operator>>=(type pos)                   noexcept       { bitset.operator>>=(static_cast<size_t>(pos)); return *this; }
        bitmask& operator<<=(type pos)                   noexcept       { bitset.operator<<=(static_cast<size_t>(pos)); return *this; }
        bitmask& operator^=(const bitset_type& right)    noexcept       { bitset.operator^=(right); return *this; }
        bitmask& operator^=(const bitmask& right)           noexcept    { operator^=(right.get());  return *this; }
        bitmask& operator|=(const bitset_type& right)    noexcept       { bitset.operator|=(right); return *this; }
        bitmask& operator|=(const bitmask& right)           noexcept    { operator|=(right.get());  return *this; }
        bitmask& operator&=(const bitset_type& right)    noexcept       { bitset.operator&=(right); return *this; }
        bitmask& operator&=(const bitmask& right)           noexcept    { operator&=(right.get());  return *this; }
    ...
    constexpr uint64_t to_bitmask(_ETy0 first)
    {
        return 1Ui64 << static_cast<uint64_t>(first);
    }

    template<typename _ETy0, typename... _ETy1>
    constexpr uint64_t to_bitmask(_ETy0 first, _ETy1... vals)
    {
        return (1Ui64 << static_cast<uint64_t>(first)) | to_mask(vals...);
    }
   
    template<class _Ty>
    [[nodiscard]] constexpr size_t array_size(const _Ty&)
    {
        using std::extent_v;

        constexpr size_t size = extent_v<_Ty>;
        if constexpr (!size)
            return extent_v<decltype(_Ty::_Elems)>;
        else return size;
    }
}