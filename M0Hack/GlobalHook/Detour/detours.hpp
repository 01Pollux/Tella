...
template<class Func>
static inline PVOID _GetAddressOfMemberFunction(Func fn) { return *reinterpret_cast<PVOID*>(&fn); }


#define DETOUR_CREATE_MEMBER(RETURN, NAME, ...) \
		class NAME##_CLASS; \
...
#define DETOUR_GET_MEMBER(CLASS) \
		reinterpret_cast<CLASS*>(this)

#define DETOUR_CREATE_LINK_TO_MEMBER(NAME) \
        &reinterpret_cast<PVOID&>(NAME##_CLASS::NAME##_ACTUAL), _GetAddressOfMemberFunction(&(NAME##_CLASS::NAME))

#define DETOUR_LINK_TO_MEMBER(NAME, ADDRESS) \
		do { \
            NAME##_CLASS::NAME##_ACTUAL = *reinterpret_cast<NAME##_FN*>(&ADDRESS);	\
		    DetourTransactionBegin(); \
		    DetourUpdateThread(GetCurrentThread()); \
		    DetourAttach(DETOUR_CREATE_LINK_TO_MEMBER(NAME)); \
		    DetourTransactionCommit(); \
        } while (false)

#define DETOUR_UNLINK_FROM_MEMBER(NAME) \
		do { \
            DetourTransactionBegin(); \
		    DetourUpdateThread(GetCurrentThread()); \
		    DetourDetach(DETOUR_CREATE_LINK_TO_MEMBER(NAME)); \
		    DetourTransactionCommit();  \
        } while (false)



#define DETOUR_CREATE_STATIC(RETURN, NAME, ...) \
        using NAME##_FN = RETURN(*)(__VA_ARGS__); \
        static NAME##_FN NAME##_ACTUAL; \
        RETURN NAME(__VA_ARGS__)

#define DETOUR_CREATE_LINK_TO_STATIC(NAME) \
        &reinterpret_cast<PVOID&>(NAME##_ACTUAL), NAME

#define DETOUR_LINK_TO_STATIC(NAME, ADDRESS) \
       do { \
            NAME##_ACTUAL = *reinterpret_cast<NAME##_FN*>(&ADDRESS);	\
		    DetourTransactionBegin(); \
		    DetourUpdateThread(GetCurrentThread()); \
		    DetourAttach(DETOUR_CREATE_LINK_TO_STATIC(NAME)); \
		    DetourTransactionCommit();  \
       } while (false)

#define DETOUR_UNLINK_FROM_STATIC(NAME) \
		do { \
            DetourTransactionBegin(); \
		    DetourUpdateThread(GetCurrentThread()); \
		    DetourDetach(DETOUR_CREATE_LINK_TO_STATIC(NAME)); \
		    DetourTransactionCommit();  \
        } while (false)

#define DETOUR_STATIC_CALL(NAME, ...) \
...
public:
    IDetour(void* addr, void* callback)
    {
        try_attach(addr, callback);
    }

    bool try_attach(void* addr, void* callback)
    {
        ...
        return (IsSet = DetourTransactionCommit() == ERROR_SUCCESS);
    }
...
public:
    IDetour() = default;
    IDetour(IDetour&) = delete;  IDetour& operator=(const IDetour&) = delete;
    IDetour(IDetour&&) = delete; IDetour& operator=(IDetour&&) = delete;
};
